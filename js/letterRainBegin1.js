let letterRainBegin = function(p) {
    let textNum = 300;
    let minSpeed = 0.1;
    let maxSpeed = 4.2;
    ...
    p.draw = function() {
      ...
      for (let i = 0; i < textNum; i++) {
        ...
        for (let j = 0; j < currentText.length; j++) {
          p.push();
          p.translate(xPos, yPos + j * verticalSpacing); 
          p.scale(1 * (speeds[i] - minSpeed) / (maxSpeed - minSpeed));//调整字号
          p.textFont(customFont);
...
let p1 = new p5(letterRainBegin, 'letterRainBegin');
